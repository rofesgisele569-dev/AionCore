-- SALE_INVOICE_CHARGE_DETAILS_INFO (10265)
-- YTLC_SALES_INVOICE → Sales Invoice 费用明细
-- 参数: :E_INVOICE_NBR
...
FROM
    ...
    SELECT 
        EVENT_INST_ID,
        LISTAGG(VOUCHER_CODE || '(' || TO_CHAR(ROUND(discount_charge / -10000, 4), 'FM999999990.0000') || ')', '; ')
            WITHIN GROUP (ORDER BY discount_charge DESC) AS DISCOUNT_INFO
    FROM EVENT_CHARGE_DISCOUNT
    GROUP BY EVENT_INST_ID
