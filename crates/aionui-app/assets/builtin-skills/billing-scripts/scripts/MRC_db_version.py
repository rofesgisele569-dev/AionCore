def main(r):
    ...
    # bundle mrc logic, added at 20260623
    # if it is bundle virtual subscriber, then MRC = 0
    bundleParentSubsId = r.event.GetSubsBundleRelation()
    print("****bundleParentSubsId****", bundleParentSubsId)
    if bundleParentSubsId == 0:
        mrcCharge = 0
    # if it is bundle member, then use bundle subs plan mrc
    elif bundleParentSubsId > 0:
        bundleSubsPlanId = r.event.GetSubsPlanId(bundleParentSubsId,currentTime)
        print("****bundleSubsPlanId****", bundleSubsPlanId)
        bundleProdSpecId = r.event.GetIndepProdSpecId(bundleParentSubsId)
        print("****bundleProdSpecId****", bundleProdSpecId)
        bundleSubsPlanMrc = float(r.event.GetPricePlanAttrValueBySubsPlanId('EXP_HNCNC_RENTAL_FEE',currentTime,bundleSubsPlanId,bundleProdSpecId) or '0')
        print("****bundleProdSpecId****", bundleProdSpecId)
        mrcCharge = bundleSubsPlanMrc
    else:
        print("****NoBundleRelation****")
